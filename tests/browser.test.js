...
const server = new patchpack.PatchPack({
  "State": ["users", "objects"],
  "User": ["name", "info"],
  "Object": ["id", "name", "foo"],
})

const client = new patchpack.PatchPack()
...
describe("Add new user to state", () => {
  ...
  test("should be equal patch and decoded patch", () => {
    state.users[id] = { name }
    const patch = { op: "add", path: `/users/${id}`, value: { name } }
...
describe("Replace value", () => {
  ...
  test("should be equal patch and decoded patch", () => {
    ...
    const patch = { op: "replace", path: `/objects/4`, value }
    const encoded = server.encodePatch(patch)
...
describe("Delete client from state", () => {
  ...
  test("should be equal patch and decoded patch", () => {
    ...
    const patch = { op: "remove", path: `/users/${id}` }
    const encoded = server.encodePatch(patch)
